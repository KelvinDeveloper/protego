<tbody>
    @if( count( $Values ) > 0 )
        ...
        @endforeach
    @else
        <tr class="record-empty">
            <td colspan="{{ $Model->total['columns'] + 1 }}">
                Não existe registros salvos. Para criar um novo registro <a href="/{{ str_singular($Model->getTable()) }}/new">clique aqui</a>.
            </td>
        </tr>
    @endif
</tbody>